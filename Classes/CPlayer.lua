...
function CPlayer:Apply(old)
	if self.Model ~= "" and (not old or self.Model ~= old.Model) then
		...
		if not self._Entity then
			CONSOLE_AddMessage("FAILED TO CREATE "..self.Name)
		end
		...
		ENTITY.SetSynchroString(self._Entity,"CPlayer")
		ENTITY.EnableNetworkSynchronization(self._Entity,true,false,0,self.ClientID,3)

		ENTITY.PO_SetMovedByExplosions(self._Entity,true)
		ENTITY.PO_SetFriction(self._Entity,0.7)

		MDL.SetAnim(self._Entity,"idle",true)
		MDL.SetMeshVisibility(self._Entity,"rl",false)
...
function CPlayer:OnDamage(damage,killer,attack_type,x,y,z,nx,ny,nz)
	...
	if MPCfg.GameState ~= GameStates.Playing and (attack_type == AttackTypes.OutOfLevel or attack_type == AttackTypes.ConsoleKill) then --and MPCfg.GameState ~= GameStates.Finished
	...
	if self and self.HasPentagram and attack_type ~= AttackTypes.OutOfLevel and attack_type ~= AttackTypes.ConsoleKill then
		self._score = nil
		if Game.GMode ~= GModes.SingleGame then
...
function CPlayer:FindFreeRespawnPoint(last,always)
	...
	if cnt > 0 then
		...
		-- RESPAWN FROM ANY MONSTER SPAWN - FFA ON SP MAPS
		allareas,totalcount = GObjects:GetElementsWithFieldValue("_Name","MonstersSpawnPoint*")
		if totalcount > 0 then
			local i = math.random(1,totalcount)
			return true,allareas[i].Pos.X,allareas[i].Pos.Y,allareas[i].Pos.Z
		else
			-- RESPAWN FROM CAMERA VIEW - SP COOP
			return true,Lev._orgPos.X,Lev._orgPos.Y,Lev._orgPos.Z
		end
	end
...
function CPlayer:Common_UpdateStats(checkOnly,deadID,killerID,attack_type,score)
	...
	if ds then
		...
		if txt and table.getn(txt) > 0 then
			...
			if ks and ks.Name then
				...
				txt = "$PLAYER was killed by a monster"
				txt = string.gsub(txt,"$PLAYER",ds.Name)
			end
		...
		if ks and ks ~= ds then
			if MPGameRules[MPCfg.GameMode].Teams and (attack_type == AttackTypes.TeleFrag
			or MPGameRules[MPCfg.GameMode].Teams and MPCfg.TeamDamage) and ks.Team == ds.Team then
	...
	if checkOnly then
		--CONSOLE_AddMessage("checkOnly determined. Please tell Blowfish")
