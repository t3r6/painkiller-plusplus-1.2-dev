...
function PSpectatorControler:Init()
	...
	ENTITY.PO_Create(self._entCam,BodyTypes.Sphere,0.3,ECollisionGroups.Normal) -- InsideItems
...
function PSpectatorControler:PostRender(delta)
	...
	HUD.PrintXY(-1,cmy+2,cameramode,Cfg.DefaultFont,0,0,0,36)
	HUD.PrintXY(-1,cmy,cameramode,Cfg.DefaultFont,160,160,160,36)
	...
	if not (MPCfg.GameMode == "Last Man Standing" and (MPCfg.GameState == GameStates.Playing or MPCfg.GameState == GameStates.Finished)) then
		--HUD.PrintXY(w-HUD.GetTextWidth(Languages.Texts[726])-10*w/1024+1,h-30*h/768+1,Languages.Texts[726],Cfg.DefaultFont,10,10,10,26*h/480)
		--HUD.PrintXY(w-HUD.GetTextWidth(Languages.Texts[726])-10*w/1024,h-30*h/768,Languages.Texts[726],Cfg.DefaultFont,230,161,97,26*h/480)
	end

	local ps = Game.PlayerStats[self.player]
	if ps and self.mode ~= 0 then
		local pnx = w/2-1.8*HUD.GetTextWidth(HUD.StripColorInfo(ps.Name))/2
		local pny = 125*h/768
		HUD.PrintXY(-1,pny+3,HUD.StripColorInfo(ps.Name),Cfg.DefaultFont,0,0,0,120)
		HUD.PrintXY(-1,pny,ps.Name,Cfg.DefaultFont,255,255,255,120)
...
function PSpectatorControler:Float()
	...
	--CONSOLE_AddMessage("PSpectatorControler:Float()")
...
function PSpectatorControler:Pivot()
	if not Cfg.SpecRange then Cfg.SpecRange = 10 end
  if not Cfg.SpecHeight then Cfg.SpecHeight = 0.8 end
	if not MOUSE.IsLocked() then return end
	local dx,dy = MOUSE.GetDelta()
	--if Cfg.InvertMouse then dy = - dy end
	local ps = Game.PlayerStats[self.player]
	if ps and ps._Entity and ps._Entity ~=0 and ps.Spectator == 0 and ps._animproc then
		...
		local v = Vector:New(0,0,Cfg.SpecRange)
...
function PSpectatorControler:Follow()
	if not Cfg.SpecRange then Cfg.SpecRange = 5 end
	if not Cfg.SpecAngle then Cfg.SpecAngle = 20 end
	if not Cfg.SpecHeight then Cfg.SpecHeight = 2.6 end
	if not MOUSE.IsLocked() then return end
	local dx,dy = MOUSE.GetDelta()
	--if Cfg.InvertMouse then dy = - dy end
	local ps = Game.PlayerStats[self.player]
	if ps and ps._Entity and ps._Entity ~=0 and ps.Spectator == 0 and ps._animproc then
		...
		local a = ENTITY.GetOrientation(ps._Entity)
		local ax = -a / (math.pi/180) - 180
		local ay = - 0 / (math.pi/180) - Cfg.SpecAngle --- ap._LastPitch / (math.pi/180) - 
		...
		CAM.SetPos(x,y,z)
		CAM.SetAngRad(ax*(math.pi/180),(Cfg.SpecAngle/2.0)*(math.pi/180),0)
		self._desiredCamAng:Set(ax*(math.pi/180),ay*(math.pi/180)+(Cfg.SpecAngle/2.0)*(math.pi/180),0)
		self._desiredCamPos:Set(x,y,z)
	end
end
--============================================================================
...
function PSpectatorControler:DrawMapview()
	...
	if(Cfg.ConfigMapView)then
		...
		,Cfg.DefaultFont,255,255,255,14*yscale)
	end
			...
			if ps and ps._Entity and ps._Entity ~=0 and ps.Spectator == 0 and ps._animproc then
				...
				if(ps.Team == 0)then
					if(yscale*py-yscale*18>0)then
						HUD.PrintXY(xscale*px,yscale*py-yscale*18,ps.Name,Cfg.DefaultFont,230,161,97,yscale*16)
					end
