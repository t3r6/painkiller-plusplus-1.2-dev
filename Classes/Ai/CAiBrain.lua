...
--============================================================================
function CAiBrain:FindTarget()
	for i,poo in GObjects.Players do
		if not self.Target then
			self.Target = poo
			self.r_closestEnemy = poo
			local x = poo._groundx
			local y = poo._groundy
			local z = poo._groundz
			self._distToNearestEnemy = Dist3D(x,y,z,self._Objactor._groundx,self._Objactor._groundy,self._Objactor._groundz)
			--CONSOLE_AddMessage("Found target "..poo._Name)
		else
			local x = poo._groundx
			local y = poo._groundy
			local z = poo._groundz
			local distance = Dist3D(x,y,z,self._Objactor._groundx,self._Objactor._groundy,self._Objactor._groundz)
			if distance < self._distToNearestEnemy then
				self.Target = poo
				self.r_closestEnemy = poo
				self._distToNearestEnemy = distance
			end
		end
	end
end
			...
			if aiParams.huntPlayer and not self._hunted then
				if aiParams.seeThruWalls then
					if self._distToNearestEnemy < aiParams.viewDistance360 then		-- dokonczyc
						self.r_closestEnemy = self.Target
						self._hunted = true
						self._seeEnemy = true
					end
				else
					if ENTITY.SeesEntity(actor._Entity, self.Target._Entity) then
						self.r_closestEnemy = self.Target
						self._hunted = true
						self._seeEnemy = true
					end
				...
				self._enemyLastSeenPoint.X = self.Target._groundx
				self._enemyLastSeenPoint.Y = self.Target._groundy
				self._enemyLastSeenPoint.Z = self.Target._groundz
				self._enemyLastSeenOrientation = self.Target._orientation
				self._enemyLastSeenVelocity.X = self.Target._velocityx
				self._enemyLastSeenVelocity.Y = self.Target._velocityy
				self._enemyLastSeenVelocity.Z = self.Target._velocityz
				self._enemyLastSeenVelocitySpeed = self.Target._velocity
			else
				if aiParams.seeThruWalls then
					if self._distToNearestEnemy < aiParams.viewDistance360 then		-- dokonczyc
						self.r_closestEnemy = self.Target
...
function CAiBrain:RotateHead(delta)
	...
	if (self.r_closestEnemy and self._seeEnemy) or actor.AiParams.forceRotateHead then
		if actor.AiParams.forceRotateHead then          -- UGLY HACK
			for i,v in GObjects.Players do
				self.r_closestEnemy = v
			end
		end
		if not self.r_closestEnemy then return end
...
function CAiBrain:WeaponFire(yDest,lrSrcZ, lrSrcY)
	...
	PlayLogicSound("FIRE",srcx,srcy,srcz,10,25,self.Target)		-- fake, ze player
	...
	if playLogicRichochet then
		if math.random(100) < 50 then
			PlayLogicSound("RICOCHET",xcol2,ycol2,zcol2,5,10,self.Target)		-- fake, ze player
		end
