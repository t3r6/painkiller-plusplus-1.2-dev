...
function Hud:DrawTeamScores(indicateID)
	...
	local scorepanel = {x = Cfg.TeamScoresX, y = Cfg.TeamScoresY, w = 0.2, d = 0.11}
	local bluescore = 0
	local redscore = 0
	local highlight = 1
	...
	if(false and highlight==1)then
		...
		-- bottom middle horizontal
		HUD.DrawQuadRGBA(Hud._matGreenPix,	w*scorepanel.x+border,h*scorepanel.y+outerh-border,innerw,	border,255,255,255,255)
	elseif(false and highlight==0)then
		-- top middle horizontal
	...
	HUD.DrawQuadRGBA(	Hud._matRedPix,		w*scorepanel.x, h*scorepanel.y + outerh,					w*scorepanel.w/2,		outerh,255,255,255,255)

	local ts1x = w*scorepanel.x + border  
	local ts2x = w*scorepanel.x + border 
	local ts1y = h*scorepanel.y + border 
	local ts2y = ts1y + outerh

	if Cfg.TeamScoresShadow then
		HUD.PrintXY(ts1x+2,ts1y+Cfg.TeamScoresShadowLevel,redscore,font,0,0,0,mediumfont)
		HUD.PrintXY(ts2x+2,ts2y+Cfg.TeamScoresShadowLevel,bluescore,font,0,0,0,mediumfont)
	end

	if highlight==1 then
		HUD.PrintXY(ts1x,ts1y,redscore,font,255,255,255,mediumfont)
		HUD.PrintXY(ts2x,ts2y,bluescore,font,127,127,127,mediumfont)
	elseif highlight==0 then
		HUD.PrintXY(ts1x,ts1y,redscore,font,127,127,127,mediumfont)
		HUD.PrintXY(ts2x,ts2y,bluescore,font,255,255,255,mediumfont)
	else
		HUD.PrintXY(ts1x,ts1y,redscore,font,255,255,255,mediumfont)
		HUD.PrintXY(ts2x,ts2y,bluescore,font,255,255,255,mediumfont)
	end
