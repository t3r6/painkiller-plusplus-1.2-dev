...
function Console:Cmd_INTERPOLATION(enable)    
    enable = tonumber(enable)    
    if enable == nil then 
        CONSOLE_AddMessage("Interpolation 0/1  (disables/enables weapon rendering)") 
        CONSOLE_AddMessage("Interpolation is currently "..tostring(Cfg.Interpolation))
        return
    else            
    	Cfg.Interpolation = enable    
  	end
end


function Console:Cmd_INTERPOLATIONTIMENUDGE(enable)    
    enable = tonumber(enable)    
    if enable == nil then 
        CONSOLE_AddMessage("InterpolationTimenudge is currently "..tostring(Cfg.InterpolationTimenudge))
    	return
    end            
    Cfg.InterpolationTimenudge = enable         
end

function Console:Cmd_INTERPOLATIONTOLERANCE(enable)    
    enable = tonumber(enable)    
    if enable == nil then 
        CONSOLE_AddMessage("InterpolationTolerance is currently "..tostring(Cfg.InterpolationTolerance))
    		return
    end            
    Cfg.InterpolationTolerance = enable         
end

function Console:Cmd_DRAWLATENCYGRAPH(enable)    
    enable = tonumber(enable)    
    if enable == nil then 
        CONSOLE_AddMessage("DrawGraphLatency 0/1  (disables/enables weapon rendering)") 
        CONSOLE_AddMessage("DrawGraphLatency is currently "..tostring(Cfg.DrawGraphLatency))
        return
    end            
    if enable == 1 then Cfg.DrawGraphLatency = true  end    
    if enable == 0 then Cfg.DrawGraphLatency = false end        
end

function Console:Cmd_INTERPOLATIONDELTADRIFT(enable)    
    enable = tonumber(enable)    
    if enable == nil then 
        CONSOLE_AddMessage("InterpolationDeltaDrift 0/1  (disables/enables weapon rendering)") 
        CONSOLE_AddMessage("InterpolationDeltaDrift is currently "..tostring(Cfg.InterpolationDeltaDrift))
        return
    end            
    if enable == 1 then Cfg.InterpolationDeltaDrift = true  end    
    if enable == 0 then Cfg.InterpolationDeltaDrift = false end        
end


function Console:Cmd_INTERPOLATIONNOSMOOTH(enable)    
    enable = tonumber(enable)    
    if enable == nil then 
        CONSOLE_AddMessage("InterpolationNoSmooth 0/1  (disables/enables weapon rendering)") 
        CONSOLE_AddMessage("InterpolationNoSmooth is currently "..tostring(Cfg.InterpolationNoSmooth))
        return
    end            
    if enable == 1 then Cfg.InterpolationNoSmooth = true  end    
    if enable == 0 then Cfg.InterpolationNoSmooth = false end        
end
